...
def login(request):
    if request.method == "POST":
        ...
        try:
            ...
            request.session['User_AP']=obj.id
            if obj.userid=='admin':
                return redirect('viewdetails')
            else:
                request.session['username']=obj.userid
...
def admin_bookdetails(request):
    obj = Book_Details.objects.all()
    return render(request,'admin_bookdetails.html',{'object': obj})
...
def check_user(request):
    username=request.session['username']
    user_data=request.session['User_AP']
    obj_details=Registermodel.objects.get(id=user_data)   
    return render(request, 'check_user.html', {'object': obj_details,'username':username})